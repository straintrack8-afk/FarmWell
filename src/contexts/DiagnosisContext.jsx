...
export const AGE_GROUPS = [
    ...
    { id: 'sows', name: 'Sows', icon: '🐖', description: 'Breeding females' },
...
export function DiagnosisProvider({ children }) {
    ...
    const getAgeGroupNames = (ageId) => {
        const mapping = {
            'newborn': ['Piglets: 0 to 7 days', 'Newborn (0-7 days)'],
            'suckling': ['Piglets: 8 days to weaning', 'Suckling (0-3 weeks)'],
            'weaned': ['Weaners: 15 to 56 days', 'Weaned (3-8 weeks)'],
            'growers': ['Growers / finishers', 'Growers (2-4 months)'],
            'finishers': ['Growers / finishers', 'Finishers (4-6 months)'],
            'sows': ['Sows / gilts / boars', 'Sows'],
            'boars': ['Sows / gilts / boars', 'Boars'],
            'all': ['All Ages'],
        };
                ...
                if (matchesSelected) {
                    disease.symptoms.forEach(symptom => {
                        ...
                        const isAlreadySelected = selectedSymptoms.some(ss =>
                            ss.toLowerCase() === symptom.toLowerCase() ||
