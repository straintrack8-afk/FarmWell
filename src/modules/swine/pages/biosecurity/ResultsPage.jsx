...
function ResultsPage() {
    ...
    // Get assessment from either location state (from history) or context (from current assessment)
    const { currentAssessment } = useBiosecurity();
    const assessment = location.state?.assessment || currentAssessment;

    useEffect(() => {
        if (assessment) {
            const items = getAllCriticalActionItems(assessment, language);
            setCriticalItems(items);

            // Calculate scores dynamically to ensure accuracy
            const overall = calculateOverallScore(assessment, language);
            const external = calculateExternalScore(assessment, language);
            const internal = calculateInternalScore(assessment, language);
            ...
            // Enterprise features
            const certStatus = getCertificationStatus(overall, language);
            setCertificationStatus(certStatus);

            const breakdown = calculateCategoryBreakdown(assessment, language);
    ...
    return (
        <div className="container print-container" style={{ paddingBottom: '4rem', maxWidth: '1000px', margin: '0 auto' }}>
                    ...
                    <div style={{ textAlign: 'center', borderRight: '1px solid #eee' }}>
                        ...
                        {certificationStatus && (
                            <div style={{
                                ...
                                background: scoreAnalysis.interpretation.color,
                                color: 'white',
                                borderRadius: '0.5rem',
                                fontWeight: '700',
                                fontSize: '1rem',
                                ...
                                {certificationStatus.badge} {certificationStatus.status}
                            </div>
                        )}
                    </div>
            ...
            {/* Category Breakdown (Enterprise Feature) */}
            {categoryBreakdown && (
