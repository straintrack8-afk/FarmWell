...
function LayerResultsPage() {
    ...
    useEffect(() => {
        ...
        if (assessmentId && (!assessmentData || !answers || Object.keys(answers).length === 0)) {
            console.log('[DEBUG] LayerResultsPage - Loading assessment by ID:', assessmentId);
            
            // Set current assessment ID
            setCurrentAssessmentId(assessmentId);
            
            // Load assessment data
            const savedAssessment = loadAssessment(assessmentId);
            if (savedAssessment) {
                console.log('[DEBUG] LayerResultsPage - Loaded assessment:', {
                    id: savedAssessment.id,
                    answersCount: Object.keys(savedAssessment.answers || {}).length,
                    hasMetadata: !!savedAssessment.metadata
                });
                
                // TODO: Dispatch to context to load this assessment
                // For now, we'll need to trigger context reload
                window.location.reload(); // Temporary fix
            } else {
                console.error('[DEBUG] LayerResultsPage - Assessment not found:', assessmentId);
            }
        }
    }, [searchParams, assessmentData, answers]);

    // Trigger calculation when assessment data is loaded
    useEffect(() => {
        if (assessmentData && answers) {
            console.log('[DEBUG] LayerResultsPage - Assessment data loaded:', {
...
export default LayerResultsPage;
