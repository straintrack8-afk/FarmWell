                ...
                <div className="fw-mod-content">
                    ...
                    <div className="fw-bio-stats-grid">
                        <div className="fw-bio-stat-card">
                            <div className="fw-bio-stat-label">{t('poultry.breeder.landing.totalAssessments') || 'Total'}</div>
                            <div className="fw-bio-stat-value">{stats.total}</div>
                            <div className="fw-bio-stat-sub">{stats.completed} {t('poultry.breeder.landing.completed') || 'completed'}</div>
                        </div>
                        <div className="fw-bio-stat-card">
                            <div className="fw-bio-stat-label">{t('poultry.breeder.landing.lastScore') || 'Last Score'}</div>
                            <div className={`fw-bio-stat-value ${stats.lastScore !== null ? 'green' : 'na'}`}>
                                {stats.lastScore !== null ? `${stats.lastScore}%` : 'N/A'}
                            </div>
                            <div className="fw-bio-stat-sub">{t('poultry.breeder.landing.latestCompleted') || 'latest'}</div>
                        </div>
                        <div className="fw-bio-stat-card">
                            <div className="fw-bio-stat-label">{t('poultry.breeder.landing.goodAssessments') || 'Good'}</div>
                            <div className="fw-bio-stat-value green">{stats.goodAssessments}</div>
                            <div className="fw-bio-stat-sub">
                                {stats.completed > 0 ? Math.round((stats.goodAssessments / stats.completed) * 100) : 0}% {t('poultry.breeder.landing.ofCompleted') || 'of completed'}
                            </div>
                        </div>
                        <div className="fw-bio-stat-card">
                            <div className="fw-bio-stat-label">{t('poultry.breeder.landing.poorAssessments') || 'Poor'}</div>
                            <div className="fw-bio-stat-value red">{stats.poorAssessments}</div>
                            <div className="fw-bio-stat-sub">{t('poultry.breeder.landing.requiresAttention') || 'requires attention'}</div>
                        </div>
                    </div>

                    <button className="fw-bio-action-btn" onClick={handleStartNewAssessment}>
                        {t('poultry.breeder.landing.startNewAssessment') || '+ Start New Assessment'}
                    </button>

                    <div className="fw-welcome-section-label">
                        {t('poultry.breeder.landing.recentAssessments') || 'Recent Assessments'}
                    </div>
                    ...
                    {savedAssessments.length === 0 ? (
                        <div className="fw-bio-empty">
                            ...
                            <div className="fw-bio-empty-title">{t('poultry.breeder.landing.noAssessmentsYet') || 'No Assessments Yet'}</div>
                            <div className="fw-bio-empty-sub">{t('poultry.breeder.landing.noAssessmentsText') || 'Start your first breeder assessment'}</div>
                            <button className="fw-bio-action-btn" style={{ marginTop: 4 }} onClick={handleStartNewAssessment}>
                                {t('poultry.breeder.landing.startFirstAssessment') || 'Start First Assessment'}
                            </button>
                        </div>
                    ) : (
                        savedAssessments.map((assessment) => {
                            ...
                            return (
                                <div
                                    ...
                                    onClick={() => isCompleted
                                        ? (setCurrentAssessmentId(assessment.id), navigate(`/poultry/breeder-assessment/results?id=${assessment.id}`))
                                ...
                                >
                                    ...
                                    <div className="fw-bio-assess-info">
                                        ...
                                        <div className="fw-bio-assess-date">
                                            {dateStr} · {isCompleted ? (t('poultry.breeder.landing.completed') || 'Completed') : `${answeredCount}/${totalQuestions}`}
                                        </div>
                                    </div>
