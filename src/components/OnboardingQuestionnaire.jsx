...
async function notifyDeveloper(data, lang) {
  ...
  const message = [
    ...
    `📰 Tips News : ${data.newsletterTips ? 'Ya' : 'Tidak'}`,
    `🔔 Prod News : ${data.newsletterProduct ? 'Ya' : 'Tidak'}`,
    `✅ Consent   : ${data.consentGiven ? 'Diberikan' : 'Tidak'}`,
    '',
    'RAW JSON',
    '━━━━━━━━━━━━━━━━━━━━━━━━━━━━━',
  ...
  try {
    await fetch('https://api.web3forms.com/submit', {
      ...
      body: JSON.stringify({
        ...
        newsletter_tips: data.newsletterTips ? 'Yes' : 'No',
        newsletter_product: data.newsletterProduct ? 'Yes' : 'No',
        consent_given: data.consentGiven ? 'Yes' : 'No',
...
// ── Total steps helper ──────────────────────────────────────────
// Poultry: Q1 → Q2(sub) → Q3 → Q4 → Q5 → Q6 → Q7 → Q8(email) = 8 steps
// Swine  : Q1 → Q3 → Q4 → Q5 → Q6 → Q7 → Q8(email)           = 7 steps
const TOTAL_STEPS_POULTRY = 8;
const TOTAL_STEPS_SWINE   = 7;

export default function OnboardingQuestionnaire({ onComplete }) {
  const { t, language } = useTranslation();
  const lang = language; // 'en' | 'id' | 'vi'

  const [step, setStep]       = useState(0);  // 0 = intro, 1-7 = questions, 8 = email, 9 = complete
  ...
  const [newsletterTips, setNewsletterTips] = useState(false);
  const [newsletterProduct, setNewsletterProduct] = useState(false);
  ...
  const handleFinish = async () => {
    ...
    setSubmitting(true);
    const popLabel = RANGE_LABELS[lang]?.[rangeIdx] ?? RANGE_LABELS.en[rangeIdx];
    const payload = {
  ...
  return (
    <div className="oq-container">
        ...
        {/* ── STEP 8 : Email + Newsletter + Consent ── */}
        {step === 8 && (
          <div className="oq-step">
            <p className="oq-step-label">{stepLabel}</p>
            <h2 className="oq-question">{t('onboarding.emailTitle')}</h2>
            <p className="oq-desc">{t('onboarding.emailDesc')}</p>

            {/* Email input */}
            <div className="oq-field-wrap">
              <label className="oq-field-label">{t('onboarding.emailLabel')}</label>
              <div className="oq-input-wrap">
                <span className="oq-input-icon">✉</span>
                <input
                  type="email"
                  className={`oq-input${emailError ? ' oq-input--error' : ''}`}
              ...
              {emailError && <p className="oq-field-error">{emailError}</p>}
              <p className="oq-field-hint">{t('onboarding.emailOptional')}</p>
            </div>

            {/* Newsletter */}
            <div className="oq-newsletter-wrap">
              <p className="oq-newsletter-title">{t('onboarding.newsletterTitle')}</p>

              <label className="oq-checkbox-card">
                <input type="checkbox" checked={newsletterTips} onChange={e => setNewsletterTips(e.target.checked)} />
                <div className="oq-checkbox-content">
                  <div className="oq-checkbox-top">
                    <span className="oq-checkbox-icon">📰</span>
                    <div>
                      <div className="oq-checkbox-label">{t('onboarding.newsletter1Label')}</div>
                      <div className="oq-checkbox-desc">{t('onboarding.newsletter1Desc')}</div>
                    </div>
                  </div>
                </div>
                <div className="oq-checkbox-indicator" />
              </label>

              <label className="oq-checkbox-card">
                <input type="checkbox" checked={newsletterProduct} onChange={e => setNewsletterProduct(e.target.checked)} />
                <div className="oq-checkbox-content">
                  <div className="oq-checkbox-top">
                    <span className="oq-checkbox-icon">🔔</span>
                    <div>
                      <div className="oq-checkbox-label">{t('onboarding.newsletter2Label')}</div>
                      <div className="oq-checkbox-desc">{t('onboarding.newsletter2Desc')}</div>
                    </div>
                  </div>
                </div>
                <div className="oq-checkbox-indicator" />
            ...
            {/* Unsubscribe note */}
            <p className="oq-unsub-note">
              <span>💬</span> {t('onboarding.unsubNote')}
            </p>

            {/* Consent */}
            <label className="oq-consent-wrap">
              <input type="checkbox" checked={consentChecked} onChange={e => setConsentChecked(e.target.checked)} />
              <div className="oq-consent-box">
                <div className="oq-consent-check-icon">{consentChecked ? '✓' : ''}</div>
                <p className="oq-consent-text">{t('onboarding.consentText')}</p>
              </div>
            </label>

            {/* Safety pills */}
            <div className="oq-safety-row">
              <span className="oq-safety-item">🔐 {t('onboarding.safety1')}</span>
              <span className="oq-safety-item">🚫 {t('onboarding.safety2')}</span>
              <span className="oq-safety-item">✉ {t('onboarding.safety3')}</span>
            </div>

            <div className="oq-nav">
              <button className="oq-btn-back" onClick={() => setStep(7)}>{t('onboarding.back')}</button>
              <button
                className="oq-btn-next"
                disabled={
                  submitting ||
                  !consentChecked ||
